...
class MultiVoyageOptimizer:
    ...
    def _evaluate_single_plan(
        ...
        # Technical benchmarks per vessel class
        class_params = {
            "Handysize": (12.0, 22.0, 14000.0, 18000.0, 35000.0),   # speed, fuel, hire, dem, typ_dwt
            "Supramax": (12.5, 27.0, 17000.0, 20000.0, 55000.0),
            "Panamax": (13.0, 32.0, 20000.0, 24000.0, 75000.0),
            "Capesize": (13.5, 52.0, 28000.0, 35000.0, 175000.0),
        }
        ...
        for i, parcel_mt in enumerate(spec.cargo_per_voyage):
            ...
            speed, fuel_tpd, hire_usd, dem_rate, typ_dwt = class_params.get(
                v_class, (13.0, 30.0, 18000.0, 22000.0, 75000.0)
            )
        ...
        try:
            mc_res = self.risk_service.simulate_monte_carlo({
                ...
                "expected_wait_days": dest_wait_days,
                "sea_distance_nm": sea_dist,
                "service_speed_knots": v_spec.service_speed_knots,
                "fuel_consumption_t_day": v_spec.fuel_consumption_mt_day,
            ...
            logger.debug(f"Monte Carlo simulation fallback: {e}")
