...
# Vessel class standard dwt mapping
VESSEL_CLASS_DWT = {
    "Handysize": 35000,
    "Supramax": 58000,
    "Ultramax": 64000,
...
class PortMovingAverageBaseline:
    ...
    def __init__(self, window_size: int = 7):
        ...
        self.port_averages: Dict[str, float] = {}

    def fit(self, df_congestion: pd.DataFrame, port_col: str = "port", target_col: str = "average_waiting_days"):
        grouped = df_congestion.groupby(port_col)[target_col].tail(self.window_size)
        self.port_averages = df_congestion.groupby(port_col)[target_col].mean().to_dict()

    def predict(self, port_id: str) -> float:
        return float(self.port_averages.get(port_id, 2.5))
...
class CongestionPredictor:
    ...
    def fit(self, df_congestion: pd.DataFrame, df_weather: Optional[pd.DataFrame] = None) -> None:
        ...
        rmse = float(np.sqrt(np.mean((y - preds_p50) ** 2)))
        self.metrics = {"MAE": round(mae, 3), "RMSE": round(rmse, 3)}
        self.is_fitted = True
        logger.info(f"CongestionPredictor fitted successfully. Metrics: {self.metrics}")
    ...
    def predict_congestion(
        ...
        if not self.is_fitted:
            ...
            return CongestionPrediction(
                ...
                model_used="Queuing_Theory_Baseline"
            )
