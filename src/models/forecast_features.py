...
def add_momentum(
    ...
    out = df.copy()
    for p in periods:
        lag_val = out[target_col].shift(p)
        out[f"momentum_{p}"] = out[target_col] - lag_val
        out[f"momentum_pct_{p}"] = ((out[target_col] - lag_val) / (lag_val + 1e-6)) * 100.0
    return out
...
class FreightFeatureBuilder:
    ...
    def build_features(
        ...
        if "indices" in exogenous_data:
            ...
            # Add index lags and momentum to prevent contemporaneous lookahead during forward prediction
            if "bdi" in idx_df.columns:
                idx_df["bdi_lag_1"] = idx_df["bdi"].shift(1)
                idx_df["bdi_momentum_7"] = idx_df["bdi"] - idx_df["bdi"].shift(7)
