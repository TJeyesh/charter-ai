...
def seed_datasets(out_dir: Path):
    ...
    for d, b, c, p, s, h in zip(dates, ts["bdi"], ts["bci"], ts["bpi"], ts["bsi"], ts["bhsi"]):
        rows_indices.append({"date": d, "index_name": "BDI", "value": round(float(b), 1), "source": "BALTIC_EXCHANGE_HISTORICAL"})
        rows_indices.append({"date": d, "index_name": "BCI", "value": round(float(c), 1), "source": "BALTIC_EXCHANGE_HISTORICAL"})
        rows_indices.append({"date": d, "index_name": "BPI", "value": round(float(p), 1), "source": "BALTIC_EXCHANGE_HISTORICAL"})
        rows_indices.append({"date": d, "index_name": "BSI", "value": round(float(s), 1), "source": "BALTIC_EXCHANGE_HISTORICAL"})
        rows_indices.append({"date": d, "index_name": "BHSI", "value": round(float(h), 1), "source": "BALTIC_EXCHANGE_HISTORICAL"})
    ...
    for d, cl, ir, gr in zip(dates, ts["coal"], ts["iron_ore"], ts["grain"]):
        rows_comm.append({"date": d, "commodity_name": "thermal_coal", "price": round(float(cl), 2), "currency": "USD", "unit": "per_tonne", "source": "GLOBALCOAL_HISTORICAL"})
        rows_comm.append({"date": d, "commodity_name": "iron_ore", "price": round(float(ir), 2), "currency": "USD", "unit": "per_tonne", "source": "PLATTS_HISTORICAL"})
        rows_comm.append({"date": d, "commodity_name": "grain", "price": round(float(gr), 2), "currency": "USD", "unit": "per_tonne", "source": "USDA_HISTORICAL"})
    ...
    for d, v, m in zip(dates, ts["vlsfo"], ts["mgo"]):
        for p in ports:
            ...
            rows_bunker.append({"date": d, "location": p, "fuel_type": "VLSFO", "price_usd_mt": round(float(v * p_adj), 2), "source": "BUNKERWORLD_HISTORICAL"})
            rows_bunker.append({"date": d, "location": p, "fuel_type": "MGO", "price_usd_mt": round(float(m * p_adj), 2), "source": "BUNKERWORLD_HISTORICAL"})
    ...
    for d, u in zip(dates, ts["usdinr"]):
        rows_econ.append({"date": d, "indicator_name": "USD_INR", "value": round(float(u), 3), "unit": "INR", "source": "RBI_REFERENCE"})
        rows_econ.append({"date": d, "indicator_name": "INDIA_MANUFACTURING_PMI", "value": 57.2, "unit": "index_points", "source": "S_AND_P_GLOBAL"})
    ...
    for i, d in enumerate(dates):
        ...
        for p, (base_w, base_wait, base_occ) in all_ports_congestion.items():
            ...
            rows_cong.append({
                ...
                "source": "PORT_AUTHORITY_HISTORICAL"
            })
            ...
            rows_weather.append({
                ...
                "source": "METEOROLOGICAL_HISTORICAL"
            })
    ...
    for cfg in routes_config:
        ...
        for d, r in zip(dates, rates):
            rows_freight.append({
                ...
                "currency": "USD",
                "unit": "per_tonne",
                "source": "BALTIC_ROUTE_HISTORICAL"
            })
